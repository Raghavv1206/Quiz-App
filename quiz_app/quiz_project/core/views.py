...
def start_quiz(request, quiz_id):
    ...
    questions = quiz.question_set.all()  # Optional: may be used later
...
def admin_dashboard(request):
    context = {
        ...
        'top_quizzes': Quiz.objects.annotate(num_attempts=Count('attempts')).order_by('-num_attempts')[:5],
    }
...
@login_required
def start_quiz(request, quiz_id):
    # 1. Fetch quiz or 404 if not found
    quiz = get_object_or_404(Quiz, id=quiz_id)
    ...
    # 3. Fetch random set of questions for the quiz
    questions = Question.objects.filter(quiz=quiz).order_by('?')

    # 4. Render template with quiz + questions
    return render(request, 'core/quiz_attempt.html', {
        'quiz': quiz,
        'questions': questions,
